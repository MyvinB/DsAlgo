...
public class MergeSort {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(mergeSort2(t)));
    }
...
// Revision - 31/10/25
    ...
    public static int[] mergeSort2(int[] nums) {
        mergeSortDivider(0,nums.length-1,nums);
        return nums;
    }

    public static void mergeSortDivider(int low,int high,int[] nums){
        if(low>=high) return;
        int mid = (high+low)/2;
        mergeSortDivider(low,mid,nums);
        mergeSortDivider(mid+1,high,nums);
        mergeIt(low,mid,high,nums);
    }

    public static void mergeIt(int low,int mid,int high, int[] nums){
        int left = low;
        int right = mid+1;
